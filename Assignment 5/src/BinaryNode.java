public class BinaryNode<T> {
	...
	public String toString() { // task4
		return toString("");
	}

	private String toString(String help) {// private helper function to use recursion
		String FinalPic = "";
		String space = help + "  ";// the depth that i want
		if (left != null) {
			FinalPic = FinalPic + left.toString(space);// left side
		}
		FinalPic = FinalPic + help + data;// printing the picture and the depth after that the information
		FinalPic = FinalPic + "\n";// getting down line
		// System.out.println();
		if (right != null) {
			FinalPic = FinalPic + right.toString(space);/// right side
		}
		return FinalPic;
	}
